...
group_by(sf, org_uuid) %>% 
  ...
  left_join(select(org, org_uuid = uuid, total_funding_usd)) %>% 
...
inv <- sf %>% 
  ...
  right_join(inv, by = c("full_investor_uuids" = "uuid")) %>% 
  rename(uuid = "full_investor_uuids")
```
